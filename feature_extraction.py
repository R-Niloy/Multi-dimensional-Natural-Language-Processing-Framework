...
with open('labeled.csv', 'w', newline='') as csvfile:
    fieldnames = ['id', 'label', 'T_Words/Turn', 'C_Words/Turn']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    count = 0
    writer.writeheader()
    for at in range(len(id)):
      writer.writerow({'id': id[count], 'label': label[count], 'T_Words/Turn': T_avg_Length[count], 'C_Words/Turn': C_avg_Length[count]})
      count = count + 1
